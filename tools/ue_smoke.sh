...
timeout 600 "$UE_ROOT/Engine/Binaries/Linux/UnrealEditor" "$REPO/unreal/SchizoGame.uproject" -game -nullrhi -unattended \
	-ExecCmds="sim.Dump, sim.Accept the_outsiders_first_days, sim.Quests active, sim.Talk the captain of the prisoner transport, sim.AdvanceDays 1, sim.AdvanceHours 12, sim.Dump, quit" -log > "$LOG" 2>&1
RC=$?
TEXT="$(tr -d '\0' < "$LOG")"
fail=0
