...
- `cmake -S kernel -B build && cmake --build build -j4 && ctest --test-dir build --output-on-failure`: 25/25 passed.
- `-DCMAKE_CXX_FLAGS="-fsanitize=address,undefined -g"` in `build-asan`, full ctest with `UBSAN_OPTIONS=halt_on_error=1`: 25/25 passed, no sanitizer reports.
