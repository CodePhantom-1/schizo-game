...
- **Your `SimDayNight` gained `SetWeatherBlend(Dust, Rain, Overcast, Fog)` (Stage V batch 5) — with all zeros it changes nothing.** `ASimAtmosphere` calls it every frame from the kernel's weather: dust thickens your haze (up to x8, ochre), rain and cloud grey it and dim the sun, fog x4. A clear day with no drought measured the same as before (imgdiff 0.59 % against a 0.57 % run-to-run floor). The night sky (real stars, `sim.Zodiac 1` for the zodiac's figures) is in `ASimAtmosphere`, not in your file. New console commands: `sim.Weather <clear|hot|scorching|sandstorm|rain|fog|auto>` (a visual override) and `sim.Zodiac 0/1`. Wet ground darkens `M_Scatter` and `M_Terrain`; `M_Building` gets its wet term after the colour-pin fix. Sound is not in yet (it waits for a Freesound API key).
