...
| A5 | **Developer console for every system**: `sim.AdvanceHours`, `sim.Give`, `sim.SetNeed`, `sim.Teleport <place>`, `sim.Spawn <npc>`, `sim.SetStanding`, `sim.Favour`, `sim.Crime`, `sim.Festival`, `sim.Weather`, `sim.Kill`, `sim.Dump <system>` | Each system in stages B–T can be tested in seconds |
